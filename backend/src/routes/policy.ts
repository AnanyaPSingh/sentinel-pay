...
router.post("/:institutionPda/policies", async (req: Request, res: Response) => {
  ...
  try {
    ...
    await runQueryExec(
      `UPDATE institutions SET policy_count = policy_count + 1, updated_at = ? WHERE institution_pda = ?`,
      [now, institutionPda]
    );
...
router.get("/:institutionPda/policies", async (req: Request, res: Response) => {
  const { institutionPda } = req.params;

  try {
    const policies = await runQuery("SELECT * FROM policies WHERE institution_pda = ?", [institutionPda]);
...
router.get("/:institutionPda/policies/:policyId", async (req: Request, res: Response) => {
...
router.patch("/:institutionPda/policies/:policyId", async (req: Request, res: Response) => {
  const { institutionPda, policyId } = req.params;
  ...
  try {
    const result = await runQueryExec(
      "UPDATE policies SET is_active = ?, updated_at = ? WHERE institution_pda = ? AND policy_id = ?",
      [isActive ? 1 : 0, now, institutionPda, Number(policyId)]
    );
